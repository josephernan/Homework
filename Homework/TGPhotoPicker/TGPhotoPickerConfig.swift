...
class TGPhotoPickerConfig {
    ...
    /** 选择框显示的位置*/
    var checkboxPosition: TGCheckboxPosition = .bottomRight{
        didSet{
            cacheNumberImage()
        }
    }
    ...
    /** 移除类型,同选择类型*/
    var removeType: TGCheckboxType = .circle{
        didSet{
            switch removeType {
            case .heart,.star:
                removeType = .circle
    ...
    /** Alert样式*/
    var useCustomActionSheet: Bool = false
    ...
    /** 选择框类型（样式） 8种 */
    var checkboxType: TGCheckboxType = .circle{
        didSet{
            switch checkboxType {
    ...
    /** 选择数量达到上限时的提示文字, #为占位符*/
    var errorImageMaxSelect = "已達圖片選擇上限"
    
    /** 拍摄标题*/
    var cameraTitle = "Open Camera"
    
    /** 选择标题*/
    var selectTitle = "Choose from library"
    ...
    /** 相机在选择照片时显示*/
    var showCarmeraInSelectPhoto: Bool = true
    
    /** 选择类型，照片、Live、视频、全部*/
    var selectKind: TGSelectKind = .onlyPhoto{
        didSet{
    ...
    /** 指示器在预览界面的显示位置*/
    var indicatorPosition: TGIndicatorPosition = .inBottomBar//.top//.inBottomBar
    
    /** 显示预览按钮*/
    var isShowPreviewButton: Bool = false
    
    /** 预览标题*/
    var previewBottonTitle: String = "預覽"
    ...
    var onlySelectPhotoOrVideo: Bool = true
